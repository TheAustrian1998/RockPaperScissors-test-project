...
contract RockPaperScissors {
    ...
    function createMatch(address defender, uint256 bet, bytes32 move) public {
        //Attacker creates a match, attacker leaves 2% of bet as collateral
        bytes32 matchId = keccak256(abi.encodePacked(block.number, msg.sender, defender));
        uint256 deadline = block.timestamp + 7 days;
        IERC20(RPSToken).transferFrom(msg.sender, address(this), bet + (bet * 2 / 100));
    ...
    function refundMatch(bytes32 matchId) public {
        ...
        IERC20(RPSToken).transfer(msg.sender, (_match.bet * 2) + (_match.bet * 2 / 100));
    }
}